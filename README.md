...
## Kubernetes Config Explained: Deployments & Services


### 🔑 Key Concepts
* **YAML Structure:** Every file consists of **Metadata** (labels/names), **Specification** (desired state), and **Status** (actual state generated by K8s).
* **Self-Healing:** Kubernetes constantly compares the `spec` with the `status` to ensure the cluster matches the desired configuration.
* **Connectivity:** * **Labels & Selectors:** Services use selectors to "find" and route traffic to Pods marked with specific labels.
    * **Port Mapping:** The Service `port` connects to the Container `targetPort`.
* **Deployment Blueprint:** Uses a `template` section to define how Pods should be created, including the image and ports.

### 🛠️ Common Commands
* `kubectl apply -f [file].yaml` - Create or update resources.
* `kubectl get pod -o wide` - View Pod status and internal IP addresses.
* `kubectl describe service [name]` - Verify Service-to-Pod endpoints.
...
* **Connectivity:** * **Labels & Selectors:** Services use selectors to "find" and route traffic to Pods marked with specific labels.
    * **Port Mapping:** The Service `port` (entry point) connects to the Container `targetPort` (app listener).
...
## 🚀 Application Deployment: MongoDB & Mongo Express

### 🔑 Key Concepts
* **Secret Management:** Sensitive data (passwords) is stored in **Secrets** using Base64 encoding to keep them out of plain-text YAML.
* **External Configuration:** Non-sensitive data (DB URLs) is stored in **ConfigMaps**, allowing for centralized updates.
* **Service Types:**
    * **ClusterIP (Internal):** Used for MongoDB so only internal apps can reach the database.
    * **LoadBalancer (External):** Used for Mongo Express to provide an entry point for web browsers.


### 🛠️ Common Commands
* `echo -n 'password' | base64` - Generate encoded strings for Secrets.
* `kubectl logs [pod-name]` - Check application logs to verify database connectivity.
* `minikube service [service-name]` - Get the public URL for an external service in a local environment.
